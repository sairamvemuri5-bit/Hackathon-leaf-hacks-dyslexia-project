...
# --- Resilient Gemini caller: pings the API up to 3 times with backoff ---
import time as _time

def gemini_post(client, url, payload, attempts=3):
    """POST to Gemini, retrying transient failures (timeouts, 429, 5xx).
    Does NOT retry 400/401/403/404 — those won't fix themselves."""
    last = None
    for i in range(attempts):
        try:
            r = client.post(url, json=payload, headers={"Content-Type": "application/json"})
            if r.status_code == 200 or r.status_code in (400, 401, 403, 404):
                return r
            print(f"Gemini attempt {i + 1}/{attempts}: HTTP {r.status_code} — retrying...")
            last = r
        except Exception as e:
            print(f"Gemini attempt {i + 1}/{attempts} error: {e} — retrying...")
            last = e
        if i < attempts - 1:
            _time.sleep(1.5 * (i + 1))
...
DEFAULT_GEMINI_KEY = "AQ.Ab8RN6IVIqYZZw8qhT6A5cu2dI_Wu7s_1plP-LkbPbwQLFbNIA"
...
def make_pollinations_url(prompt: str) -> str:
    ...
    style_suffix = "professional 2D educational textbook diagram, clean vector style, flat colors, clear labels, no text in image, no letters, no words, no logos"
...
# Gemini native image generation (much higher quality than Pollinations).
# Falls back to a Pollinations URL if the model/quota is unavailable.
CARTOON_SUFFIX = "professional 2D educational textbook diagram, clean vector style, flat colors, clear labels, no text in image, no letters, no words, no logos"

def generate_gemini_image(prompt: str, api_key: str) -> Optional[str]:
    try:
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key={api_key}"
        payload = {
            "contents": [{
                "parts": [{"text": prompt + CARTOON_SUFFIX}]
            }],
            "generationConfig": {
        ...
        with httpx.Client(timeout=60.0) as client:
            response = gemini_post(client, url, payload)
            if response.status_code != 200:
                print(f"Gemini image gen returned {response.status_code}; falling back to Pollinations.")
                return None
        ...
        print(f"Gemini image gen failed: {e}; falling back to Pollinations.")
    return None

def render_panel_images(panels: List[dict], api_key: Optional[str]):
    """Fill each panel's image_url with a Gemini-generated image (concurrently), or Pollinations fallback."""
    from concurrent.futures import ThreadPoolExecutor

    def one(panel):
        img = generate_gemini_image(panel.get("prompt", ""), api_key) if api_key else None
        panel["image_url"] = img or make_pollinations_url(panel.get("prompt", ""))

    with ThreadPoolExecutor(max_workers=3) as ex:
        list(ex.map(one, panels))
...
def split_text_into_scenes(text: str) -> List[dict]:
    ...
    for idx, sentence in enumerate(sentences):
        ...
        if keywords:
            ...
            prompt = f"cute simple children's cartoon of {search_terms}"
        else:
            prompt = f"cute simple children's cartoon showing {clean_sentence[:50]}"
...
# Gemini Voice Transcription Logic (for the Study Buddy orb in desktop mode)
def perform_transcribe_logic(audio_base64: str, user_api_key: Optional[str] = None) -> str:
    ...
    payload = {
        "contents": [{
            "parts": [
                ...
                {
                    "text": "Transcribe this audio recording of a student speaking. Return ONLY the transcribed words as plain text, with normal punctuation. Do not add any commentary, headers, or explanations. If the audio is silent or unintelligible, return an empty string."
                }
            ]
        }]
    }
    ...
    with httpx.Client(timeout=30.0) as client:
        response = gemini_post(client, url, payload)
        if response.status_code != 200:
            raise Exception(f"Gemini transcription API failed: {response.text}")
        ...
        try:
            ...
            raise Exception(f"Failed to parse Gemini transcription response: {str(e)}")
...
def generate_comic_logic(text: str, mode: str = "comic", user_api_key: Optional[str] = None) -> dict:
        ...
        try:
            if mode == "diagram":
                ...
                2. Design a single, high-fidelity labeled/annotated academic diagram or technical infographic that visually summarizes the entire topic.
                   The 'prompt' for this diagram must describe an adorable, VERY SIMPLE children's picture-book cartoon that a 5 year old would love.
                   Use big simple shapes, smiley friendly elements, flat bright colors, and thick rounded outlines. Keep the layout clear and uncluttered so labels can be placed on top.
                   The drawing must be completely text-free (no letters, numbers, words, or labels drawn in the image itself).
                   Always start the prompt with "cute simple cartoon for a 5 year old, preschool picture book style, text-free illustration showing..." to maintain style.
                ...
                2. Structure the text into a chronological comic book storyboard of 3 to 6 panels for a dyslexic child.
                   ...
                   To ensure the image generator (Pollinations AI) creates highly relevant drawings, the 'prompt' for each panel must describe an adorable, VERY SIMPLE children's picture-book cartoon that a 5 year old would love: one big clear idea per panel, big simple shapes, smiley friendly characters (like a happy sun, a smiling cloud, a cheerful plant), flat bright colors, and thick rounded outlines.
                   The drawings must be completely text-free (no letters, numbers, words, or labels drawn in the image itself).
                   Always start the prompt with "cute simple cartoon for a 5 year old, preschool picture book style, text-free illustration showing..." to maintain style.
            ...
            with httpx.Client(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    render_panel_images(panels, api_key)
    ...
    if mode == "diagram":
        ...
        prompt = f"cute simple cartoon for a 5 year old, preschool picture book style, text-free illustration showing {topic}"
...
async def api_transcribe(request: TranscribeRequest):
    try:
        text = perform_transcribe_logic(request.audio_base64, request.apiKey)
...
class DesktopApi:
    ...
    def transcribe_audio(self, audio_base64, apiKey=None):
        try:
            text = perform_transcribe_logic(audio_base64, apiKey)
            return {"text": text}
        except Exception as e:
